...
/**
 * GRIT 파트 — 매니페스토를 읽고 나서 벽이 열린다.
 *
 * 3분할(/)의 GRIT 칸으로 들어오는 화면이다. 왼쪽 위 화살표로 다시 3분할로 나간다.
 *
 * 스크롤 한 줄에 두 인터랙션이 차례로 걸린다.
 * 1. 문단 셋이 다가와 읽히고 GRIT 심볼 모양으로 타 사라진 뒤, 그 자리에 로고가 선다.
 * 2. 이어 내리면 벽 사진 프레임이 열려 화면을 먹고, 같은 스크롤로
 *    "Be Experimental"이 "GRIT"으로 녹아 바뀐다. 다 열리면 눌러서 3분할로 나간다.
 *
 ...
 *
 * (site) 그룹에 넣지 않는다 — 화면 하나가 통째로 인터랙션이라
 * 흐름에 놓이는 nav가 sticky 무대를 밀어낸다. 나가는 길은 아래 화살표가 맡는다.
 */
...
export default function GritPage() {
  ...
  return (
    <div data-screen="grit" className="relative">
